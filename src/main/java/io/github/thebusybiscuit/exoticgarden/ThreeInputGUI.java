...
public abstract class ThreeInputGUI extends SlimefunItem implements InventoryBlock, EnergyNetComponent {
    ...
    private void constructMenu(BlockMenuPreset preset) {
        for (int i : border) {
            preset.addItem(i, new CustomItemStack(Material.BLACK_STAINED_GLASS_PANE, " "), (player, i6, itemStack, clickAction) -> false);
        }
        for (int i : inputBorder) {
            preset.addItem(i, new CustomItemStack(Material.WHITE_STAINED_GLASS_PANE, " "), (player, i5, itemStack, clickAction) -> false);
        }
        ...
        for (int i : subSlotSign) {
            preset.addItem(i, new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&e副输出槽", "", "&7副输出槽通常会输出机器的副产物", "&7有些副产物极其有用甚至非常珍贵"), (player, i2, itemStack, clickAction) -> false);
        }
        for (int i : mainSlotSign) {
            preset.addItem(i, new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&c主输出槽", "", "&7主输出槽输出机器的常规产品"), (player, i1, itemStack, clickAction) -> false);
        }
